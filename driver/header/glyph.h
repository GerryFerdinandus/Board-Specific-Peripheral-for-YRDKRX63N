...
typedef struct _glyph_version_info
{
    ...
    uint32_t nVersionAPIMajor ;
    uint32_t nVersionAPIMinor ;
    uint32_t nVersionIMPLMajor ;
    uint32_t nVersionIMPLMinor ;
    uint32_t nImplementationID ;
} T_glyphVersionInfo ;
...
typedef struct {
    ...
    void (*iCommandSend)(uint8_t cCommand);
    void (*iDataSend)(uint8_t cData);
} T_Comm_API ;
...
T_glyphError GlyphString(T_glyphHandle aHandle, const uint8_t * aString, uint32_t aLength);
...
T_glyphError GlyphSetContrast(T_glyphHandle aHandle, uint32_t nContrast) ;
